...
public: // Function declarations
  ...
  // Ho modificato la funzione initialize() per scegliere io manualmente la temperatura che voglio
  void initialize(double temperature = -1.);          // Initialize system properties
  ...
  // Aggiungo io
  void setTemp(double T) {_temp = T;}; 
  double getTemp() const {return _temp;};
  int getType() const {return sim_type;};
  double get_H() const {return _H;};

  void print_last_two_columns(const std::string& input_filename, const std::string& output_filename, double const temperature);
