...
void System :: initialize(double temperature){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      if (temperature == -1.){
        ...
        _beta = 1.0/_temp;
        coutf << "TEMPERATURE= " << _temp << endl;
        coutf << "#TEMPERATURA = " << _temp << "INIZIALIZZATA A MANO" <<  endl;
      }
...
// TO BE FIXED IN EXERCISE 6
  ...
  //Qui riporto l'energia al quadrato, così tutto il processo mi resitutuirà il valor medio di H^2
...
// TO BE FIXED IN EXERCISE 6
  ...
  if (_measure_chi) {
    ...
    chi = _beta * chi * chi / double(_npart);
...
void System :: averages(int blk){
  ...
  if (_measure_cv && _measure_tenergy){
    ...
    average = pow(_beta,2) * (_average(_index_cv) - pow(_average(_index_tenergy),2))*_npart; // calcolo la vera grandezza che mi interessa
...
void System :: print_last_two_columns(const std::string& input_filename, const std::string& output_filename, double const temperature) {
    std::ifstream infile(input_filename);
    std::ofstream outfile(output_filename, std::ios::app); // modalità append

    if (!infile.is_open()) {
        std::cerr << "Errore nell'apertura del file di input: " << input_filename << std::endl;
        return;
    }
    if (!outfile.is_open()) {
        std::cerr << "Errore nell'apertura del file di output: " << output_filename << std::endl;
        return;
    }
    ...
    if (columns.size() < 4) {
        std::cerr << "file: " << input_filename << std::endl;
        std::cerr << "La riga finale ha meno di 4 colonne. Formato non valido." << std::endl;
        return;
    }

    // Prendi le ultime due colonne
    std::string col3 = columns[columns.size() - 2];
    std::string col4 = columns[columns.size() - 1];

    // Scrivile nel file di output
    outfile << temperature << " " << col3 << " " << col4 << "\n";
