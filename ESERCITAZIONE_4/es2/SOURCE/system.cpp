...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    //Metto tutte le velocità a delta di dirac lungo le varie direzioni x, -x, y, -y, z, -z
    double _v_star = sqrt(3.0*_temp);
  ...
  if(plot_iniz){
  // ==================== Salva velocità iniziali per istogramma ===================={
    ofstream fout("../OUTPUT/pofv_iniz.dat");
    if(!fout.is_open()){
        cerr << "Errore: impossibile aprire v_init.dat" << endl;
    } else {
        for(int i=0; i<_npart; i++){
            double vx = _particle(i).getvelocity(0);
            double vy = _particle(i).getvelocity(1);
            double vz = _particle(i).getvelocity(2);
            double v_mag = sqrt(vx*vx + vy*vy + vz*vz);  // modulo velocità
            fout << v_mag << endl;
        }
        fout.close();
        cout << "Velocità iniziali salvate in ../OUTPUT/pofv_iniz.dat" << endl;
    }
  }
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0*sqrt(_temp)/(double)_n_bins_v; // TO BE FIXED IN EXERCISE 4, metto la radice FATTO
        _measure_pofv = true;
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      // ES 4.2 HO MODIFICATO LE BOX AFFINCHÈ VENGA OCCUPATA SOLO METÀ BOX IN OGNI DIREZIONE
      _particle(i).setposition(0, this->pbc(_side(0)*x*0.5, 0));
      _particle(i).setposition(1, this->pbc(_side(1)*y*0.5, 1));
      _particle(i).setposition(2, this->pbc(_side(2)*z*0.5, 2));
...
void System :: measure(){ // Measure properties
  ...
  if(_measure_pofv){
    double _v_norm = 0.;
    for(int i = 0; i < _npart; i++){
      _v_norm = sqrt(dot(_particle(i).getvelocity(), _particle(i).getvelocity()));
      bin = static_cast<int>(_v_norm / _bin_size_v); //tronca tutti i decimali
      if(bin < _n_bins_v) _measurement(_index_pofv + bin) += 1.0;
    }
...
void System :: averages(int blk){
  ...
  if (_measure_pofv){
    ...
    // Scrivi intestazione per il blocco
    coutf << "# Block " << blk << endl;
    ...
    for(int i = 0; i < _n_bins_v; i++){
      ...
      // Centro del bin
      double v_bin_center = (i + 0.5) * _bin_size_v;

      coutf << setw(12) << v_bin_center
            << setw(12) << average
            << setw(12) << sum_average/double(blk)
            << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    }
