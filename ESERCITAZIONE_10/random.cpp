...
void Random :: Start(int n = 1){
   int seed[4];
   int p1, p2;
   ifstream Primes("Primes");
   if (Primes.is_open()){
      for(int i = 0; i < n; i++) Primes >> p1 >> p2 ;
...
void Random :: city_circle(int number) {
   ofstream off_file;
   off_file.open("circle.out");
   if (off_file.is_open()){

      for (int i = 0; i < number; i++) {
         double theta = Rannyu() * 2. * M_PI;
         off_file << cos(theta) << " " << sin(theta) << endl;
      }

   } else cerr << "PROBLEM: Unable to open circle.out" << endl;
  off_file.close();
  return;
}

void Random :: city_box(int number) {
   ofstream off_file;
   off_file.open("box.out");
   if (off_file.is_open()){

      for (int i = 0; i < number; i++) {
         off_file << Rannyu() << " " << Rannyu() << endl;
      }

   } else cerr << "PROBLEM: Unable to open box.out" << endl;
  off_file.close();
  return;
}
