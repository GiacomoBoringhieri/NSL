...
double Chromosome::evaluateFit() {
    ...
    for (int i = 0; i < _genes.size() - 1; i++) {
        //sum += abs((_genes(i).x - _genes(i+1).x)) + abs((_genes(i).y - _genes(i+1).y));
        sum += pow((_genes(i).x - _genes(i+1).x), 2) + pow((_genes(i).y - _genes(i+1).y), 2);
    }
    // Aggiunge la distanza tra l'ultimo e il primo gene per chiudere il ciclo
    sum += pow((_genes(_genes.size()-1).x - _genes(0).x), 2) + pow((_genes(_genes.size()-1).y - _genes(0).y), 2);
    //sum += abs((_genes(_genes.size()-1).x - _genes(0).x)) + abs((_genes(_genes.size()-1).y - _genes(0).y));
    return sum;
}
...
// Ritorna il percorso come vector di indici (per MPI)
vector<int> Chromosome::getAsVector() const {
    vector<int> percorso(_genes_number);
    for (int i = 0; i < _genes_number; i++) {
        percorso[i] = _genes(i).index;
    }
    return percorso;
}

// Imposta il cromosoma da un vector di indici (per MPI)
void Chromosome::setFromVector(const vector<int>& percorso) {
    if (percorso.size() != _genes_number) {
        cerr << "ERROR: Dimensione percorso non corrisponde al numero di geni" << endl;
        return;
    }
    
    // Crea un array temporaneo per mantenere le coordinate originali
    arma::field<Gene> temp_genes(_genes_number);
    
    // Copia le coordinate originali in un array temporaneo indicizzato per index
    for (int i = 0; i < _genes_number; i++) {
        temp_genes(_genes(i).index) = _genes(i);
    }
    
    // Riorganizza i geni secondo il nuovo ordine
    for (int i = 0; i < _genes_number; i++) {
        int new_index = percorso[i];
        _genes(i) = temp_genes(new_index);
    }
}

// Ritorna l'indice del gene alla posizione i
int Chromosome::getGeneIndex(int i) const {
    if (i < 0 || i >= _genes_number) {
        cerr << "ERROR: Indice fuori range in getGeneIndex" << endl;
        return -1;
    }
    return _genes(i).index;
}

// Imposta l'indice del gene alla posizione i
void Chromosome::setGeneIndex(int i, int index) {
    if (i < 0 || i >= _genes_number) {
        cerr << "ERROR: Indice fuori range in setGeneIndex" << endl;
        return;
    }
    _genes(i).index = index;
}