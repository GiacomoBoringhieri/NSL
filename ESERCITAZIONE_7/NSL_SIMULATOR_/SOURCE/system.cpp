...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        bin = ceil(dr/_bin_size); // capisco in quale bin rientra la distanza, CAPIRE BENE
        if(bin < _n_bins) {
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  // CAMBIATO IL MODO DI PRENDERE LA TEMPERATURA SE È MC O MD
    if (_measure_kenergy){
    if (_sim_type == 0){
...
void System :: averages(int blk){
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 7, da rivedere!
  if(_measure_gofr){
